...
function WoWBnB:CreateCheckbox(option, label, parent, updateFunc)
	...
	EventRegistry:RegisterCallback("WoWBnB.OnReset", function()
		UpdateOption(self.defaults[option])
	end, cb)
	return cb
end

function WoWBnB:InitializeOptions()
	-- main panel
	self.panel_main = CreateFrame("Frame")
	self.panel_main.name = "WoWBnB"
	...
	btn_reset:SetScript("OnClick", function()
		WoWBnBDB = CopyTable(WoWBnB.defaults)
		self.db = WoWBnBDB
		EventRegistry:TriggerEvent("WoWBnB.OnReset")
	end)

	InterfaceOptions_AddCategory(WoWBnB.panel_main)
...
function WoWBnB.UpdateIcon(value)
	if not WoWBnB.mushroom then
		WoWBnB.mushroom = CreateIcon("interface/icons/inv_mushroom_11", 64, 64, UIParent)
		WoWBnB.mushroom:SetPoint("CENTER")
	end
	WoWBnB.mushroom:SetShown(value)
end

-- a bit more efficient to register/unregister the event when it fires a lot
function WoWBnB:UpdateEvent(value, event)
	if value then
